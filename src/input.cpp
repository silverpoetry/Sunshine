...
#define ENABLE_LEFT_BUTTON_DELAY nullptr
   ...
   */
  void print(PSS_TOUCHPAD_PACKET packet) {
    BOOST_LOG(debug)
   ...
   */
  float from_touchpad_frame_uint16(std::uint16_t value) {
   ...
   */
  void print(PSS_TOUCHPAD_FRAME_PACKET packet) {
   ...
   * @param packet The touchpad packet.
   */
  void passthrough(std::shared_ptr<input_t> &input, PSS_TOUCHPAD_PACKET packet) {
    if (!config::input.mouse) {
      return;
    }
   ...
   * @param packet The touchpad frame packet.
   */
  void passthrough(std::shared_ptr<input_t> &input, PSS_TOUCHPAD_FRAME_PACKET packet) {
    if (!config::input.mouse || !config::input.native_touchpad_optimization) {
      return;
    }

    static_assert(platf::MAX_TOUCHPAD_FRAME_CONTACTS >= SS_TOUCHPAD_FRAME_MAX_CONTACTS, "platform touchpad frame capacity must cover the protocol maximum");
   ...
   * @return The status of the batching operation.
   */
  batch_result_e batch(PSS_TOUCHPAD_PACKET dest, PSS_TOUCHPAD_PACKET src) {
    if (dest->eventType != LI_TOUCH_EVENT_MOVE &&
        dest->eventType != LI_TOUCH_EVENT_HOVER) {
      return batch_result_e::terminate_batch;
    }
   ...
   * @param packet The touchpad frame packet.
   * @return `true` if the frame has at least one contact and all contacts are move events.
   */
  bool touchpad_frame_is_move_only(PSS_TOUCHPAD_FRAME_PACKET packet) {
   ...
   * @return The status of the batching operation.
   */
  batch_result_e batch(PSS_TOUCHPAD_FRAME_PACKET dest, PSS_TOUCHPAD_FRAME_PACKET src) {
